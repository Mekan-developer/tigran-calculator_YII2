...
<body class="bg-gray-100">
    <?php $this->beginBody() ?>
       <header class="sticky top-0">
            <div class="flex justify-between items-center h-[60px] bg-gray-800 text-white px-4">
                <div class="hidden sm:flex"></div>
                <button data-drawer-target="separator-sidebar" data-drawer-toggle="separator-sidebar" aria-controls="separator-sidebar" type="button" class="inline-flex items-center p-2 text-sm text-gray-400 rounded-lg ms-3 text-nowrap text-[14px] sm:hidden hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-gray-600">
                ...
                <div class="flex items-center space-x-4">
                    ...
                    <div x-data="{ open: false }" class="flex items-center space-x-2 cursor-pointer">
                        ...
                        <!-- Dropdown Menu -->
                        <div x-show="open" @click.away="open = false" class="absolute top-[52px] right-0 mt-2 w-48 bg-white border border-gray-300 rounded-lg shadow-lg z-10">
                            <div class="py-2">
                                <a href="<?= yii\helpers\Url::to(['user/settings']) ?>" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-200">Settings</a>
                                <a href="<?= yii\helpers\Url::to(['login/logout']) ?>" 
                                    class="flex no-underline w-[200px] items-center px-4 py-2  text-gray-700 hover:bg-gray-200 "
                                    data-method="post">
                                    <i class="fa-solid flex-shrink-0 text-size-[20px] text-gray-400 transition duration-75 group-hover:text-white fa-sign-out-alt"></i>
                                    <span class="ms-3 text-nowrap text-[14px]">Logout</span>
                                </a> 
                            </div>
                        </div>
                    </div>
        ...
        <aside 
            ...
            <div class="flex flex-col justify-between items-center h-full py-4">
                        ...
                        <?php if (Yii::$app->user->can('admin')){ ?>
                            <li>
                                <a href="<?= yii\helpers\Url::to(['user/index']) ?>" 
                                    class="flex no-underline w-[200px] items-center p-2 text-white rounded-lg hover:bg-gray-700 group <?= Yii::$app->controller->id === 'user' ? 'bg-gray-700' : '' ?>">
                                    <i class="fa-solid fa-users-gear"></i>
                                    <span class="ms-3 text-nowrap text-[14px]">Users</span>
                                </a>
                            </li>
                        <?php } ?>
                ...
                <div>
                    <a href="<?= yii\helpers\Url::to(['login/logout']) ?>" 
                        class="flex no-underline w-[200px] items-center p-2 text-white rounded-lg hover:bg-gray-700 group"
                        data-method="post">
                        <i class="fa-solid flex-shrink-0 text-size-[20px] text-gray-400 transition duration-75 group-hover:text-white fa-sign-out-alt"></i>
                        <span class="ms-3 text-nowrap text-[14px]">Logout</span>
                    </a>              
