                        ...
                        <div id="stone-fields-container">
                            <?php if (!empty($stoneModels)): ?>
                                <?php foreach ($stoneModels as $index => $stoneModel): ?>
                                    <div class="flex flex-col flex-wrap gap-4 items-center mb-3">
                                            ...
                                            <div class="flex-1">
                                                <?= $form->field($stoneModel, "[$index]stone_id")->dropDownList(
                                                    ...
                                                    [
                                                        ...
                                                        'class' => ' w-full text-[14px] border border-gray-300 rounded-sm focus:ring-yellow-500 focus:border-yellow-500'
                                                    ]
                                                )->label('Камень') ?>
                                            </div>
                                        </div>
                            ...
                            <?php if (!empty($workModels)): ?>
                                <?php foreach ($workModels as $index => $workModel): ?>
                                    <div class="flex items-center mb-3 space-x-4">
                                        ...
                                        <div class="flex-1">
                                        <?= $form->field($workModel, "[$index]work_id")->dropDownList(
                                            \yii\helpers\ArrayHelper::map(Work::find()->all(), 'id', 'work_name'),
                                            [
                                                'prompt' => 'Выберите работу',
                                                'class' => 'block w-full text-[14px] border border-gray-300 rounded-sm focus:ring-green-500 focus:border-green-500',
                                            ]
                                        )->label('Работа') ?>
                                        </div>
                                        <div class="flex-1">
                                    ...
                                    <?php if ($workModel->hasErrors('work_id')): ?>
                                        <div class="text-red-500"><?= Html::error($workModel, "[$index]work_id") ?></div>
                                    <?php endif; ?>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        stoneFieldsContainer.addEventListener('click', function (e) {
            if (e.target && e.target.id === 'add-stone-field') {
                ...
                const optionsHtml = Object.entries(stonesOptions)
                    ...
                    const newFieldHtml = `
                        <div class="stone-field-group">
                            <div class="w-full h-1 bg-gray-400 mb-4"></div>
        ...
        stoneFieldsContainer.addEventListener('click', function (e) {
            if (e.target.classList.contains('remove-field')) {
                // Remove the closest parent .stone-field-group
                const fieldToRemove = e.target.closest('.stone-field-group');
                if (fieldToRemove) {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const createNewFieldHtml = (index) => {
            return `
                <div class="flex items-center mb-3 space-x-4 work-field">
                    <button type="button" class="w-[40px] aspect-square text-white bg-green-600 rounded-sm hover:bg-green-700 remove-field">
                        -
                    </button>

                    <div class="flex-1">
                        <select name="WorkCalculation[${index}][work_id]" class="block w-full text-[14px] border border-gray-300 rounded-sm focus:ring-green-500 focus:border-green-500">
        ...
        // Fetch work options dynamically (replace with a server-side rendered JSON if needed)
        const generateWorkOptions = () => {
            const workOptions = <?= json_encode(
                \yii\helpers\ArrayHelper::map(
                    Work::find()->all(),
                    'id',
                    'work_name'
            ...
            return Object.entries(workOptions)
                .map(([value, label]) => `<option value="${value}">${label}</option>`)
                .join('');
        };
        addButton.addEventListener('click', function () {
            const index = workFieldsContainer.querySelectorAll('.work-field').length; // Get the current count of fields
            const newFieldHtml = createNewFieldHtml(index);
            workFieldsContainer.insertAdjacentHTML('beforeend', newFieldHtml);
        });

        workFieldsContainer.addEventListener('click', function (e) {
            if (e.target.classList.contains('remove-field')) {
                const fieldToRemove = e.target.closest('.work-field');
                if (fieldToRemove) {
                    fieldToRemove.remove();
                }
            }
